...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "GenerateProfitCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "GenerateProfitCoin is downloading blocks...");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 128/4; i++)
            ((unsigned int*)pdata)[i] = ByteGenerateProfitCoinerse(((unsigned int*)pdata)[i]);
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "GenerateProfitCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "GenerateProfitCoin is downloading blocks...");
