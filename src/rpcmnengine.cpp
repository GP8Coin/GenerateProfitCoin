...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  start        - Start masternode configured in GenerateProfitCoin.conf\n"
                "  start-alias  - Start single masternode by assigned alias configured in masternode.conf\n"
                "  start-many   - Start all masternodes configured in masternode.conf\n"
                "  status       - Print masternode status information\n"
                "  stop         - Stop masternode configured in GenerateProfitCoin.conf\n"
                ...
                "  vote-many    - Vote on a GenerateProfitCoin initiative\n"
                "  vote         - Vote on a GenerateProfitCoin initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CGenerateProfitCoinSecret(secret).ToString();
    }
    ...
    if(strCommand == "status")
    {
        ...
        if (pmn) mnObj.push_back(Pair("pubkey", CGenerateProfitCoinAddress(pmn->pubkey.GetID()).ToString()));
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a GenerateProfitCoin initiative (can be additionally filtered, partial match)\n"
