...
{
    widget->setMaxLength(GenerateProfitCoinAddressValidator::MaxAddressLength);
    widget->setValidator(new GenerateProfitCoinAddressValidator(parent));
...
bool parseGenerateProfitCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("GenerateProfitCoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!GenerateProfitCoinUnits::parse(GenerateProfitCoinUnits::GP8, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseGenerateProfitCoinURI(QString uri, SendCoinsRecipient *out)
{
    // Convert rev:// to GenerateProfitCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("rev://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "GenerateProfitCoin:");
    }
    QUrl uriInstance(uri);
    return parseGenerateProfitCoinURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GenerateProfitCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for GenerateProfitCoin.lnk
...
{
    return GetAutostartDir() / "GenerateProfitCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a GenerateProfitCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=GenerateProfitCoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the GenerateProfitCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add GenerateProfitCoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GenerateProfitCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  GenerateProfitCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("GenerateProfitCoin-Qt"));
