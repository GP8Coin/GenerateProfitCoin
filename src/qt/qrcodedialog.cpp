...
{
    QString ret = QString("GenerateProfitCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non GP8 unit input in lnReqAmount, we generate the URI with GP8 as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(GenerateProfitCoinUnits::format(GenerateProfitCoinUnits::GP8, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
