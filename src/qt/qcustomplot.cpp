...
QCPAxis::QCPAxis(QCPAxisRect *parent, AxisType type) :
  ...
  // scale and range:
  mRange(0, 5),
  mRangeGenerateProfitCoinersed(false),
  mScaleType(stLinear),
  mScaleLogBase(10),
...
/*!
  ...
  To invert the direction of an axis, use \ref setRangeGenerateProfitCoinersed.
*/
...
/*!
  ...
  upper part.
*/
void QCPAxis::setRangeGenerateProfitCoinersed(bool reversed)
{
  if (mRangeGenerateProfitCoinersed != reversed)
  {
    mRangeGenerateProfitCoinersed = reversed;
    mCachedMarginValid = false;
  }
}
...
/*!
  ...
  setRangeGenerateProfitCoinersed.
...
double QCPAxis::pixelToCoord(double value) const
{
  if (orientation() == Qt::Horizontal)
  {
    if (mScaleType == stLinear)
    {
      if (!mRangeGenerateProfitCoinersed)
...
double QCPAxis::coordToPixel(double value) const
{
  if (orientation() == Qt::Horizontal)
  {
    ...
    } else // mScaleType == stLogarithmic
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeGenerateProfitCoinersed ? mAxisRect->right()+200 : mAxisRect->left()-200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeGenerateProfitCoinersed ? mAxisRect->left()-200 : mAxisRect->right()+200;
      else
      {
        if (!mRangeGenerateProfitCoinersed)
  ...
  } else // orientation() == Qt::Vertical
  {
    ...
    {
      if (value >= 0 && mRange.upper < 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeGenerateProfitCoinersed ? mAxisRect->top()-200 : mAxisRect->bottom()+200;
      else if (value <= 0 && mRange.upper > 0) // invalid value for logarithmic scale, just draw it outside visible range
        return !mRangeGenerateProfitCoinersed ? mAxisRect->bottom()+200 : mAxisRect->top()-200;
      else
      {
        if (!mRangeGenerateProfitCoinersed)
...
void QCPAxis::draw(QCPPainter *painter)
{
  ...
  mAxisPainter->reversedEndings = mRangeGenerateProfitCoinersed;
...
/*!
  ...
  \li range reversed (\ref QCPAxis::setRangeGenerateProfitCoinersed)
...
void QCPAxisRect::setupFullAxesBox(bool connectRanges)
{
  ...
  xAxis2->setRangeGenerateProfitCoinersed(xAxis->rangeGenerateProfitCoinersed());
  ...
  yAxis2->setRangeGenerateProfitCoinersed(yAxis->rangeGenerateProfitCoinersed());
...
void QCPColorScaleAxisRectPrivate::draw(QCPPainter *painter)
{
  ...
  {
    mirrorHorz = mParentColorScale->mColorAxis.data()->rangeGenerateProfitCoinersed() && (mParentColorScale->type() == QCPAxis::atBottom || mParentColorScale->type() == QCPAxis::atTop);
    mirrorVert = mParentColorScale->mColorAxis.data()->rangeGenerateProfitCoinersed() && (mParentColorScale->type() == QCPAxis::atLeft || mParentColorScale->type() == QCPAxis::atRight);
  }
...
void QCPGraph::getPreparedData(QVector<QCPData> *lineData, QVector<QCPData> *scatterData) const
{
  ...
  if (mAdaptiveSampling && dataCount >= maxCount) // use adaptive sampling only if there are at least two points per pixel on average
  {
    if (lineData)
    {
      ...
      int reversedFactor = keyAxis->rangeGenerateProfitCoinersed() ? -1 : 1; // is used to calculate keyEpsilon pixel into the correct direction
      int reversedRound = keyAxis->rangeGenerateProfitCoinersed() ? 1 : 0; // is used to switch between floor (normal) and ceil (reversed) rounding of currentIntervalStartKey
...
void QCPGraph::drawError(QCPPainter *painter, double x, double y, const QCPData &data) const
{
  ...
  if (keyAxis->orientation() == Qt::Vertical)
  {
    ...
    if (mErrorType == etKey || mErrorType == etBoth)
    {
      ...
      if (keyAxis->rangeGenerateProfitCoinersed())
        qSwap(a,b);
      // draw spine:
    ...
    if (mErrorType == etValue || mErrorType == etBoth)
    {
      ...
      if (valueAxis->rangeGenerateProfitCoinersed())
        qSwap(a,b);
      // draw spine:
      if (mErrorBarSkipSymbol)
      {
...
QPointF QCPGraph::lowerFillBasePoint(double lowerKey) const
{
  ...
  } else // valueAxis->mScaleType == QCPAxis::stLogarithmic
  {
    ...
    if (keyAxis->orientation() == Qt::Vertical)
    {
      if ((valueAxis->range().upper < 0 && !valueAxis->rangeGenerateProfitCoinersed()) ||
          (valueAxis->range().upper > 0 && valueAxis->rangeGenerateProfitCoinersed())) // if range is negative, zero is on opposite side of key axis
...
const QPolygonF QCPGraph::getChannelFillPolygon(const QVector<QPointF> *lineData) const
{
  ...
  {
    // x is key
    // if an axis range is reversed, the data point keys will be descending. GenerateProfitCoinerse them, since following algorithm assumes ascending keys:
...
void QCPCurve::getCurveData(QVector<QPointF> *lineData) const
{
  ...
  double rectLeft = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyAxis->range().lower)-strokeMargin*((keyAxis->orientation()==Qt::Vertical)!=keyAxis->rangeGenerateProfitCoinersed()?-1:1));
  double rectRight = keyAxis->pixelToCoord(keyAxis->coordToPixel(keyAxis->range().upper)+strokeMargin*((keyAxis->orientation()==Qt::Vertical)!=keyAxis->rangeGenerateProfitCoinersed()?-1:1));
  double rectBottom = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().lower)+strokeMargin*((valueAxis->orientation()==Qt::Horizontal)!=valueAxis->rangeGenerateProfitCoinersed()?-1:1));
  double rectTop = valueAxis->pixelToCoord(valueAxis->coordToPixel(valueAxis->range().upper)-strokeMargin*((valueAxis->orientation()==Qt::Horizontal)!=valueAxis->rangeGenerateProfitCoinersed()?-1:1));
  int currentRegion;
...
void QCPBars::getVisibleDataBounds(QCPBarDataMap::const_iterator &lower, QCPBarDataMap::const_iterator &upperEnd) const
{
  ...
  while (it != mData->constBegin())
  {
    ...
    if (mKeyAxis.data()->orientation() == Qt::Horizontal)
      isVisible = ((!mKeyAxis.data()->rangeGenerateProfitCoinersed() && barBounds.right() >= lowerPixelBound) || (mKeyAxis.data()->rangeGenerateProfitCoinersed() && barBounds.left() <= lowerPixelBound));
    else // keyaxis is vertical
      isVisible = ((!mKeyAxis.data()->rangeGenerateProfitCoinersed() && barBounds.top() <= lowerPixelBound) || (mKeyAxis.data()->rangeGenerateProfitCoinersed() && barBounds.bottom() >= lowerPixelBound));
    if (isVisible)
      lower = it;
    else
      break;
  ...
  while (it != mData->constEnd())
  {
    ...
    if (mKeyAxis.data()->orientation() == Qt::Horizontal)
      isVisible = ((!mKeyAxis.data()->rangeGenerateProfitCoinersed() && barBounds.left() <= upperPixelBound) || (mKeyAxis.data()->rangeGenerateProfitCoinersed() && barBounds.right() >= upperPixelBound));
    else // keyaxis is vertical
      isVisible = ((!mKeyAxis.data()->rangeGenerateProfitCoinersed() && barBounds.bottom() >= upperPixelBound) || (mKeyAxis.data()->rangeGenerateProfitCoinersed() && barBounds.top() <= upperPixelBound));
    if (isVisible)
      upperEnd = it+1;
    else
      break;
...
void QCPBars::getPixelWidth(double key, double &lower, double &upper) const
{
  switch (mWidthType)
  {
    case wtAbsolute:
    {
      upper = mWidth*0.5;
      lower = -upper;
      if (mKeyAxis && (mKeyAxis.data()->rangeGenerateProfitCoinersed() ^ (mKeyAxis.data()->orientation() == Qt::Vertical)))
        qSwap(lower, upper);
      break;
    }
...
void QCPColorMap::updateLegendIcon(Qt::TransformationMode transformMode, const QSize &thumbSize)
{
  ...
  {
    bool mirrorX = (keyAxis()->orientation() == Qt::Horizontal ? keyAxis() : valueAxis())->rangeGenerateProfitCoinersed();
    bool mirrorY = (valueAxis()->orientation() == Qt::Vertical ? valueAxis() : keyAxis())->rangeGenerateProfitCoinersed();
