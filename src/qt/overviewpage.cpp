...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(GenerateProfitCoinUnits::GP8)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseDarkTheme ? QColor(255, 255, 255) : foreground);
        QString amountText = GenerateProfitCoinUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(GenerateProfitCoinUnits::formatWithUnit(nDisplayUnit, balance));
    ui->labelStake->setText(GenerateProfitCoinUnits::formatWithUnit(nDisplayUnit, stake));
    ui->labelUnconfirmed->setText(GenerateProfitCoinUnits::formatWithUnit(nDisplayUnit, unconfirmedBalance));
    ui->labelImmature->setText(GenerateProfitCoinUnits::formatWithUnit(nDisplayUnit, immatureBalance));
    ui->labelTotal->setText(GenerateProfitCoinUnits::formatWithUnit(nDisplayUnit, balance + stake + unconfirmedBalance + immatureBalance));
    //ui->labelWatchAvailable->setText(GenerateProfitCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance));
    //ui->labelWatchStake->setText(GenerateProfitCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyStake));
    //ui->labelWatchPending->setText(GenerateProfitCoinUnits::floorWithUnit(nDisplayUnit, watchUnconfBalance));
    //ui->labelWatchImmature->setText(GenerateProfitCoinUnits::floorWithUnit(nDisplayUnit, watchImmatureBalance));
    //ui->labelWatchTotal->setText(GenerateProfitCoinUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance + watchOnlyStake + watchUnconfBalance + watchImmatureBalance));
...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    //ui->labelGP8totalimg->setVisible(showWatchOnly);    // show watch-only balance separator line
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("GP8")
    updateDisplayUnit();
}
...
{
    GenerateProfitCoinGUI::gotoHistoryPage_static();
}
