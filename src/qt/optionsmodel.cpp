...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", GenerateProfitCoinUnits::GP8);
    ...
    if (!settings.contains("nAnonymizeGenerateProfitCoinAmount"))
        settings.setValue("nAnonymizeGenerateProfitCoinAmount", 1000);
    nAnonymizeGenerateProfitCoinAmount = settings.value("nAnonymizeGenerateProfitCoinAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeGenerateProfitCoinAmount"))
        SoftSetArg("-anonymizeGenerateProfitCoinamount", settings.value("nAnonymizeGenerateProfitCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeGenerateProfitCoinAmount:
            return QVariant(nAnonymizeGenerateProfitCoinAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeGenerateProfitCoinAmount:
            nAnonymizeGenerateProfitCoinAmount = value.toInt();
            settings.setValue("nAnonymizeGenerateProfitCoinAmount", nAnonymizeGenerateProfitCoinAmount);
            emit AnonymizeGenerateProfitCoinAmountChanged(nAnonymizeGenerateProfitCoinAmount);
            break;
        default:
            break;
        }
