...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", GenerateProfitCoinGUI::tr("A fatal error occurred. GenerateProfitCoin can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    if (!boost::filesystem::is_directory(GetDataDir(false)))
    {
        ...
        QMessageBox::critical(0, "GenerateProfitCoin",
                              QString("Error: Specified data directory \"%1\" does not exist.").arg(QString::fromStdString(mapArgs["-datadir"])));
        return 1;
    }

    // Process GenerateProfitCoin config
    InitializeConfigFile();

    // Application identification (must be set before OptionsModel is initialized,
    // as it is used to locate QSettings)
    app.setOrganizationName("GenerateProfitCoin");
    //XXX app.setOrganizationDomain("");
    if(GetBoolArg("-testnet", false)) // Separate UI settings for testnet
        app.setApplicationName("GenerateProfitCoin-Qt-testnet");
    else
        app.setApplicationName("GenerateProfitCoin-Qt");
    ...
    try
    {
        ...
        if(AppInit2(threadGroup))
        {
            ...
            // Shutdown the core and its threads, but don't exit GenerateProfitCoin-Qt here
