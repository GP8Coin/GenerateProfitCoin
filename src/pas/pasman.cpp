...
void CPubkeyaliasserviceMan::ProcessMessagePAS(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "pasee") { //MNengine Election Entry
        ...
        if(fAcceptable){
            ...
            // should be at least not earlier than block when 75 GenerateProfitCoin tx got PUBKEYALIASSERVICE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            GetTransaction(vin.prevout.hash, txVin_man, hashBlock);
            map<uint256, CBlockIndex*>::iterator mi = mapBlockIndex.find(hashBlock);
           if (mi != mapBlockIndex.end() && (*mi).second)
            {
                CBlockIndex* pPASIndex = (*mi).second; // block for 75 GenerateProfitCoin tx -> 1 confirmation
