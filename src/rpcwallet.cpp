...
#ifdef ENABLE_WALLET
        // Case 1: GenerateProfitCoin address and we have full public key:
        CGenerateProfitCoinAddress address(ks);
...
Value createmultisig(const Array& params, bool fHelp)
{
    ...
    CGenerateProfitCoinAddress address(innerID);
...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new GenerateProfitCoin address for receiving payments.\n"
            "If 'account' is specified (recommended), it is added to the address book \n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"        (string, optional) The account name for the address to be linked to. if not provided, the default account \"\" is used. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created if there is no account by the given name.\n"
            "\nResult:\n"
            "\"GenerateProfitCoin\"    (string) The new GenerateProfitCoin address\n"
            "\nExamples:\n"
    ...
    return CGenerateProfitCoinAddress(keyID).ToString();
}


CGenerateProfitCoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CGenerateProfitCoinAddress(account.vchPubKey.GetID());
}
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current GenerateProfitCoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"GenerateProfitCoin\"   (string) The account GenerateProfitCoin address\n"
            "\nExamples:\n"
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"GenerateProfitCoin\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"GenerateProfitCoinaddress\"  (string, required) The GenerateProfitCoin address to be associated with an account.\n"
    ...
    CGenerateProfitCoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GenerateProfitCoin address");
...
Value getaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"GenerateProfitCoin\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"GenerateProfitCoin\"  (string, required) The GenerateProfitCoin address for account lookup.\n"
            "\nResult:\n"
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"GenerateProfitCoin\"  (string) a GenerateProfitCoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CGenerateProfitCoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CGenerateProfitCoinAddress& address = item.first;
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"GenerateProfitCoin\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSent an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n"
            + HelpRequiringPassphrase() +
            "\nArguments:\n"
           "1. \"GenerateProfitCoin\"  (string, required) The GenerateProfitCoin address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in GP8 to send. eg 0.1\n"
...
Value listaddressgroupings(const Array& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"GenerateProfitCoin\",     (string) The GenerateProfitCoin address\n"
            "      amount,                 (numeric) The amount in GP8\n"
    ...
    BOOST_FOREACH(set<CTxDestination> grouping, pwalletMain->GetAddressGroupings())
    {
        ...
        {
            Array addressInfo;
            addressInfo.push_back(CGenerateProfitCoinAddress(address).ToString());
            addressInfo.push_back(ValueFromAmount(balances[address]));
            {
                LOCK(pwalletMain->cs_wallet);
                if (pwalletMain->mapAddressBook.find(CGenerateProfitCoinAddress(address).Get()) != pwalletMain->mapAddressBook.end())
                    addressInfo.push_back(pwalletMain->mapAddressBook.find(CGenerateProfitCoinAddress(address).Get())->second);
            }
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"GenerateProfitCoin\" \"message\"\n"
            "\nSign a message with the private key of an address"
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"GenerateProfitCoin\"  (string, required) The GenerateProfitCoin address to use for the private key.\n"
    ...
    CGenerateProfitCoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"GenerateProfitCoin\" ( minconf )\n"
            "\nReturns the total amount received by the given GenerateProfitCoin in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"GenerateProfitCoin\"  (string, required) The GenerateProfitCoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount in GP8 received at this address.\n"
            "\nExamples:\n"
    ...
    // GenerateProfitCoin address
    CGenerateProfitCoinAddress address = CGenerateProfitCoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GenerateProfitCoin address");
...
Value getreceivedbyaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in GP8 received for this account.\n"
            "\nExamples:\n"
...
Value movecmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 GP8 from the default account to the account named tabby\n"
            + HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 GP8 timotei to akiko with a comment and funds have 10 confirmations\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"toGenerateProfitCoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a GenerateProfitCoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toGenerateProfitCoinaddress\"  (string, required) The GenerateProfitCoin address to send funds to.\n"
            ...
            "\"transactionid\"        (string) The transaction id.\n"
            "\nExamples:\n"
            "\nSend 0.01 GP8 from the default account to the address, must have at least 1 confirmation\n"
    ...
    string strAccount = AccountFromValue(params[0]);
    CGenerateProfitCoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid GenerateProfitCoin address");
    CAmount nAmount = AmountFromValue(params[2]);
...
Value sendmany(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The GenerateProfitCoin address is the key, the numeric amount in GP8 is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    set<CGenerateProfitCoinAddress> setAddress;
    vector<pair<CScript, int64_t> > vecSend;

    int64_t totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CGenerateProfitCoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid GenerateProfitCoin address: ")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a GenerateProfitCoin address or hex-encoded public key.\n"
            "If 'account' is specified, assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of GenerateProfitCoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) GenerateProfitCoin address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"GenerateProfitCoin\"  (string) A GenerateProfitCoin address associated with the keys.\n"
    ...
    return CGenerateProfitCoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CGenerateProfitCoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CGenerateProfitCoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CGenerateProfitCoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
Value listreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in GP8 received by the address\n"
...
{
    CGenerateProfitCoinAddress addr;
    if (addr.Set(dest))
...
Value listtransactions(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"GenerateProfitCoin\",    (string) The GenerateProfitCoin address of the transaction. Not present for \n"
            ...
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in GP8. This is negative and only available for the \n"
...
Value listsinceblock(const Array& params, bool fHelp)
{
    if (fHelp)
       throw runtime_error(
            ...
            "    \"address\":\"GenerateProfitCoin\",    (string) The GenerateProfitCoin address of the transaction. Not present for move transactions (category = move).\n"
            ...
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in GP8. This is negative and only available for the 'send' category of transactions.\n"
...
Value gettransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in GP8\n"
            ...
            "      \"address\" : \"GenerateProfitCoin\",   (string) The GenerateProfitCoin address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in GP8\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending GP8\n"
            "\nArguments:\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending GenerateProfitCoin\n"
            + HelpExampleCli("walletpassphrase", "\"my pass phrase\"") +
            "\nNow we can so something like sign\n"
            + HelpExampleCli("signmessage", "\"GenerateProfitCoinaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; GenerateProfitCoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
Value settxfee(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in GP8/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new GenerateProfitCoin stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid GenerateProfitCoin stealth address."));
        return result;
    };
...
Value cclistcoins(const Array& params, bool fHelp)
{
        ...
        BOOST_FOREACH(const COutput& out, vCoins)
    {
                ...
                coutput.push_back(Pair("Address", CGenerateProfitCoinAddress(outputAddress).ToString()));
