...
int nAnonymizeGenerateProfitCoinAmount = 1000;
...
#else
    const char* pszModule = "GenerateProfitCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GP8
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GP8
    // Mac: ~/Library/Application Support/GP8
    // Unix: ~/.GP8
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GP8";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "GenerateProfitCoin.conf"));
...
void BuildConfigFile()
{
    // TODO: remove boost in favor of C++11 file lookup/handling
    boost::filesystem::path ConfPath;
    ConfPath = GetDataDir() / "GenerateProfitCoin.conf";
    FILE* ConfFile = fopen(ConfPath.string().c_str(), "w");
    ...
    fprintf(ConfFile, "port=20775\n");
    fprintf(ConfFile, "rpcport=20725\n");
    fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
    fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
    fprintf(ConfFile, "addnode=103.3.62.31:20775\n");
    fprintf(ConfFile, "addnode=192.53.122.152:20775\n");
    fclose(ConfFile);
}
...
void InitializeConfigFile()
{
    ...
    if (!streamConfig.good())
    {
        // Create GenerateProfitCoin.conf
        BuildConfigFile();
        // Then read it...
        ReadConfigFile();
    } else {
        // Read GenerateProfitCoin.conf
        ReadConfigFile();
    }
}
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "GenerateProfitCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong GenerateProfitCoin will not work properly.");
