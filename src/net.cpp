...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (GenerateProfitCoin users should be well used to small numbers with many decimal places by now :)
    ...
    while (true)
    {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            if (GetTime() - pnode->nTimeConnected > IDLE_TIMEOUT)
            {
                ...
                // Ping timeout - TODO : GenerateProfitCoiniew function
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "GenerateProfitCoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. GenerateProfitCoin is probably already running."), addrBind.ToString());
        else
