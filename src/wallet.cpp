...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CGenerateProfitCoinAddress(redeemScript.GetID()).ToString();
    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    LogPrintf("ReacceptWalletTransactions found spent coin %s GP8 %s\n", FormatMoney(wtx.GetCredit(ISMINE_ALL)), wtx.GetHash().ToString());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _(" Unable to locate enough MNengine non-denominated funds for this transaction that are not equal 1000 GP8.");
                    }
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-GenerateProfitCoin-address
                    CScript scriptChange;
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        CGenerateProfitCoinAddress addr(ckid);
        ...
        if (fDebug)
        {
            ...
            CGenerateProfitCoinAddress coinAddress(keyID);
...
bool CWallet::SendStealthMoneyToDestination(CStealthAddress& sxAddress, int64_t nValue, std::string& sNarr, CWalletTx& wtxNew, std::string& sError, bool fAskFee)
{
    ...
    CGenerateProfitCoinAddress addrTo(ckidTo);
    ...
    // -- Parse GenerateProfitCoin address
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (Params().NetworkID() == CChainParams::MAIN) {
        devopaddress = CBitcoinAddress("GdwuN3ZwMNGHEAwr2kHsBzQ1EawHuuQykn");
    ...
    if(hasPayment){
        ...
        CGenerateProfitCoinAddress address2(address1);
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CGenerateProfitCoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).EraseName(CGenerateProfitCoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CGenerateProfitCoinAddress(address).ToString());
}
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                LogPrintf("FixSpentCoins found lost coin %s GP8 %s[%d], %s\n",
            ...
            {
                LogPrintf("FixSpentCoins found spent coin %s GP8 %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue), pcoin->GetHash().ToString(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
bool CWallet::ImportPrivateKey(CGenerateProfitCoinSecret vchSecret, string strLabel, bool fRescan) {
    if (fWalletUnlockStakingOnly)
        return false;
